...
def jso():
    ...
    for dota in data["js"]["data"]:
        ...
        if dota["tv_genre_id"]=="127":
            ...
            dt.append({"title":dota["name"],"playerLabel":dota["name"],"action":"video:plugin:http://msx.benzac.de/plugins/hls.html?url="+request.base_url.replace("/json","/m3u8/")+dota["cmd"].replace("ffmpeg ","").replace("?","piyush").replace("&","aman")})
...
def playlist(link):
    ...
    req=requests.get(link.replace("aman","&").replace("piyush","?").replace("extension=ts","extension=m3u8"))
...
def tss(ts):
    ...
    final=make_response(str(requests.get("http://f17s20.alphapx.me:80/hls/"+ts).text))
